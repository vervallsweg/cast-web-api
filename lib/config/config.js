...
class Config {
    ...
    static isWindows() {
        if (Config._windows) {
            require("../log/logger").log( process.argv[1].substring(0, process.argv[1].length - 17) );
    ...
    static getConfig() {
        return new Promise(resolve => {
           ...
           Config.getLatestVersion()
               .then(value => {
                   config.api.version.latest = value;
                })
               .finally(()=>{
                   AssistantSetup.checkSetup()
                       .then(assistantSetup => {
                           config.assistant = assistantSetup;
                       })
                       .finally(()=>{
                           resolve(config);
                       });
