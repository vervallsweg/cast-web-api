...
function getSteps(number) {
	...
	var step1 = '<a class="'+active[1]+'step" href="/assistant/setup/1"><div class="content"><div class="title">Sign up</div><div class="description">Get API ID and secret</div></div></a>';
	var step2 = '<a class="'+active[2]+'step" href="/assistant/setup/2"><div class="content"><div class="title">Authorize</div><div class="description">Access to your Google account</div></div></a>';
	var step3 = '<a class="'+active[3]+'step" href="/assistant/setup/3"><div class="content"><div class="title">Save</div><div class="description">Save your authorization code</div></div></a>';
...
var step1Content = step1Content + '<button style="float: right;" class="ui right labeled icon primary button" id="save-btn"><i class="save icon"></i><div>Save</div></button><a style="float: right; display: none;" class="ui right labeled icon green button" id="next-btn" href="/assistant/setup/2"><i class="arrow right icon"></i><div>Next</div></a>';
...
var step2Content = step2Content + '<a style="float: right;" class="ui right labeled icon green button" id="next-btn" href="/assistant/setup/3"><i class="arrow right icon"></i><div>Next</div></a>';
...
var step3JS = '<script>$("#save-btn").click(function(){tokenSuccess||$.ajax("/assistant/setup/token/"+$("#token")[0].value).always(function(){$("#save-btn").removeClass("right labeled icon").addClass("loading")}).done(function(e){$("#save-btn").removeClass("loading primary red").addClass("right labeled icon green"),$("#save-btn > div").text("Done"),$("#save-btn > i").removeClass("save times").addClass("check"),tokenSuccess=!0}).fail(function(e){var s="No error message provided.";e&&e.responseJSON&&(s=e.responseJSON.error),$(".ui.ordered.fluid.steps").after("+errorPre+"+s+"+errorPost+"),$("#save-btn").removeClass("loading primary green").addClass("right labeled icon red"),$("#save-btn > div").text("Error"),$("#save-btn > i").removeClass("check save").addClass("times")})});</script>';
