...
var pre = '<html><head>'+head+'</head><body>'+mainContainerStart+'<h1>Setup Google Assistant</h1>';
...
function getSteps(number) {
	...
	var step1 = '<a class="'+active[1]+'step" href="1"><div class="content"><div class="title">Sign up</div><div class="description">Get API ID and secret</div></div></a>';
	var step2 = '<a class="'+active[2]+'step" href="2"><div class="content"><div class="title">Authorize</div><div class="description">Access to your Google account</div></div></a>';
	var step3 = '<a class="'+active[3]+'step" href="3"><div class="content"><div class="title">Save</div><div class="description">Save your authorization code</div></div></a>';

	return (pre+step1+step2+step3+post);
}
...
var step1Content = step1Content + '<button style="float: right;" class="ui right labeled icon primary button" id="save-btn"><i class="save icon"></i><div>Save</div></button><a style="float: right; display: none;" class="ui right labeled icon green button" id="next-btn" href="2"><i class="arrow right icon"></i><div>Next</div></a>';
var step1JS = '<script>$("#save-btn").click(function(){$.ajax("id/"+$("#client-id")[0].value).always(function(){$("#save-btn").removeClass("right labeled icon").addClass("loading")}).done(function(e){$.ajax("secret/"+$("#client-secret")[0].value).always(function(){$("#save-btn").removeClass("right labeled icon").addClass("loading")}).done(function(e){$("#save-btn").removeClass("loading primary red").addClass("right labeled icon green"),$("#save-btn > div").text("Done"),$("#save-btn > i").removeClass("save times").addClass("check"),window.setTimeout(function(){$("#save-btn").hide(),$("#next-btn").show()},1e3)}).fail(function(e){var s="No error message provided.";e&&e.responseJSON&&(s=e.responseJSON.error),$(".ui.ordered.fluid.steps").after('+errorPre+'+s+'+errorPost+'),$("#save-btn").removeClass("loading primary green").addClass("right labeled icon red"),$("#save-btn > div").text("Error"),$("#save-btn > i").removeClass("check save").addClass("times")})}).fail(function(e){var s="No error message provided.";e&&e.responseJSON&&(s=e.responseJSON.error),$(".ui.ordered.fluid.steps").after('+errorPre+'+s+'+errorPost+'),$("#save-btn").removeClass("loading primary green").addClass("right labeled icon red"),$("#save-btn > div").text("Error"),$("#save-btn > i").removeClass("check save").addClass("times")})});</script>';

//STEP 2
var step2Content = '<p>Click the button below to authorize cast-web-api with your Google Account.<br><br> Copy the code you received, click next and paste it in the next step.<p><a href="https://developers.google.com/assistant/sdk/guides/service/python/embed/config-dev-project-and-account" target="blank" class="ui button" id="auth-btn">Authorize</a></p>';
var step2Content = step2Content + '<a style="float: right;" class="ui right labeled icon green button" id="next-btn" href="3"><i class="arrow right icon"></i><div>Next</div></a>';
var step2JS = '<script>$.ajax("getTokenUrl/").always(function(){$("#auth-btn").addClass("loading")}).done(function(r){console.log(r.url),$("#auth-btn").removeClass("loading").attr("href",r.url)}).fail(function(r){var e="No error message provided.";r&&r.responseJSON&&(e=r.responseJSON.error),$(".ui.ordered.fluid.steps").after('+errorPre+'+e+'+errorPost+'),$("#auth-btn").removeClass("loading").addClass("red"),$("#auth-btn > div").text("Error getting auth url")});</script>';
...
var step3JS = '<script>$("#save-btn").click(function(){$.ajax("token/"+$("#token")[0].value).always(function(){$("#save-btn").removeClass("right labeled icon").addClass("loading")}).done(function(e){$("#save-btn").removeClass("loading primary red").addClass("right labeled icon green"),$("#save-btn > div").text("Done"),$("#save-btn > i").removeClass("save times").addClass("check")}).fail(function(e){var a="No error message provided.";e&&e.responseJSON&&(a=e.responseJSON.error),$(".ui.ordered.fluid.steps").after('+errorPre+'+a+'+errorPost+'),$("#save-btn").removeClass("loading primary green").addClass("right labeled icon red"),$("#save-btn > div").text("Error"),$("#save-btn > i").removeClass("check save").addClass("times")})});</script>';
